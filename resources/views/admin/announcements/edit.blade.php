        ...
        <div class="col-lg-7 col-md-9 col-12">
            <div class="card mt-5 bg-dark text-white border-0 shadow-lg p-4" style="border-radius: 16px;">
                <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label for="status" class="form-label text-muted small text-uppercase">Durum</label>
                        <select class="form-select custom-input @error('status') is-invalid @enderror"
                            ...
                            <option value="inactive" {{ old('status', $announcement->status) === 'inactive' ? 'selected' : '' }}>Pasif</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
